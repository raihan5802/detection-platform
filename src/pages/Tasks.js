...
function Tasks() {
    ...
    const handleDeleteTask = (taskId, taskName, e) => {
        e.stopPropagation(); // Prevent card click event

        if (deleteConfirmationTaskId === taskId) {
            // User confirmed deletion, proceed with deleting
            deleteTask(taskId);
        } else {
            // Show confirmation
    ...
    const deleteTask = async (taskId) => {
        ...
        try {
            const res = await fetch(`http://localhost:4000/api/tasks/${taskId}?userId=${userSession.id}`, {
                method: 'DELETE',
            });
            ...
            if (res.ok) {
                ...
                setDeleteConfirmationTaskId(null);
                alert(`Task "${result.taskName}" deleted successfully`);
            } else {
            ...
            >
                ...
                <div className="task-actions">
                    <button
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/task-info/${task.task_id}`);
                        }}
                    >
                        <FiEye /> View
                    </button>
                    ...
                    {task.access_level === 'editor' && (
                        <button
                            className={`delete-task-btn ${deleteConfirmationTaskId === task.task_id ? 'confirming' : ''}`}
                            onClick={(e) => handleDeleteTask(task.task_id, task.task_name, e)}
                        >
                            <FiTrash2 /> {deleteConfirmationTaskId === task.task_id ? 'Confirm' : 'Delete'}
                        </button>
                    )}
                </div>
            </div>
